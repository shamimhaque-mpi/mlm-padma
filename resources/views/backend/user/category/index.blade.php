...
@section('content')
    <div class="container-fluid">
        ...
        <ol class="breadcrumb mb-4 rounded-0">
            ...
            <li class="breadcrumb-item"><a href="{{route('user.category')}}">Categories</a></li>
                ...
                <div class="card mb-4 rounded-0">
                    ...
                    <div class="card-body">
                        <form action="{{route('user.category.store')}}" method="POST">
                            ...
                            <input type="hidden" name="position" value="{{count($allCategories) + 1}}" required>

                            <div class="form-group">
                                <label for="name">Name <span class="text-danger">*</span> </label>
                                <input type="text" name="category_name" class="form-control" id="categoryName" autocomplete="off" required>
                            </div>
                                ...
                                <select name="icon" class="form-control selectpicker" data-live-search="true" required>
                                    ...
                                    @if($allIcons->isNotEmpty())
                                        @foreach($allIcons as $list)
                                            <option data-icon="{{$list->class_name}}">{{$list->class_name}}</option>
                                        @endforeach
                                    @endif
                                </select>
                            </div>
                                ...
                                @foreach($allCategories as $key => $row)
                                    <tr>
                                        ...
                                        <td class="text-center">
                                            <a title="Edit" href="{{route('user.category.edit', $row->id)}}" class="btn btn-sm btn-warning">
                                                <i class="fa fa-pencil-square-o"></i></a>

                                            <a title="Delete" href="{{route('user.category.destroy', $row->id)}}"
